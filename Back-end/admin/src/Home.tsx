...
const ServiceIcon = ({ title, className = "text-[#ccff00] w-16 h-16" }: { title: string, className?: string }) => {
  switch (title) {
...
function Home() {
  ...
  return (
    <div ref={containerRef} className="bg-[#0f0f0f] text-white font-sans selection:bg-[#ccff00] selection:text-black overflow-x-hidden">
      {/* Custom Cursor */}
      <div 
        ref={cursorRef} 
        className="fixed top-0 left-0 w-8 h-8 border-2 border-[#ccff00] rounded-full pointer-events-none z-[9999] -translate-x-1/2 -translate-y-1/2 mix-blend-difference hidden md:block"
      />
      ...
      <nav className="fixed top-0 w-full z-50">
        <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
          <div className="flex items-center gap-4 group cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <div className="w-12 h-12 bg-[#ccff00] rounded-xl flex items-center justify-center group-hover:rotate-12 transition-transform duration-500 shadow-lg shadow-[#ccff00]/20">
              <Dumbbell className="text-black w-6 h-6" />
            </div>
            <span className="text-xl font-bold tracking-tighter uppercase">
              Sinen <span className="text-[#ccff00]">Fitness</span>
            </span>
          </div>
          
          <div className="hidden md:flex items-center gap-10 text-[10px] font-bold uppercase tracking-[0.2em] text-white/70">
            <button onClick={() => scrollToSection('about')} className="hover:text-[#ccff00] transition-colors">About</button>
            <button onClick={() => scrollToSection('services')} className="hover:text-[#ccff00] transition-colors">Services</button>
            <button onClick={() => scrollToSection('gallery')} className="hover:text-[#ccff00] transition-colors">Gallery</button>
            <button onClick={() => scrollToSection('pricing')} className="hover:text-[#ccff00] transition-colors">Pricing</button>
            <button onClick={() => scrollToSection('contact')} className="hover:text-[#ccff00] transition-colors">Contact</button>
          </div>

          <button 
            onClick={() => navigate('/register')}
            className="hidden sm:block bg-white/5 hover:bg-[#ccff00] hover:text-black border border-white/10 hover:border-[#ccff00] px-6 py-2.5 rounded-full font-bold text-[10px] uppercase tracking-widest transition-all duration-300"
          >
            Join Now
          </button>
        </div>
      ...
      <section ref={heroRef} className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="hero-content relative z-10 text-center px-6">
          <h1 className="text-7xl md:text-[12vw] font-black uppercase italic tracking-tighter leading-none mb-8">
            Built for <br /> <span className="text-[#ccff00]">Discipline</span>
          </h1>
          <p className="text-gray-400 text-xl md:text-2xl max-w-2xl mx-auto mb-12 italic">
            Professional training. Modern equipment. Smart membership reminders via Telegram so you never miss progress.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <button 
              onClick={() => navigate('/register')}
              className="bg-[#ccff00] text-black px-12 py-5 rounded-full font-black text-sm uppercase tracking-widest hover:scale-105 transition-transform"
            >
              Book Free Trial
            </button>
            <a href="tel:0934437205" className="flex items-center gap-3 text-white font-bold uppercase tracking-widest text-xs hover:text-[#ccff00] transition-colors">
              <Phone size={18} /> Call 0934437205
            </a>
          </div>
        </div>
      </section>
          ...
          <div className="relative w-full lg:w-1/2 order-2 lg:order-1">
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-8">
              <div className="bg-zinc-900/80 backdrop-blur-sm p-8 rounded-[2rem] border border-white/5 relative overflow-hidden group">
                <div className="absolute -right-4 -bottom-4 text-[#ccff00]/10 group-hover:scale-110 transition-transform">
                  <Dumbbell size={80} />
                </div>
                <div className="text-3xl md:text-4xl font-black text-[#ccff00] mb-2 italic relative z-10">200 ETB</div>
              ...
              <div className="bg-zinc-900/80 backdrop-blur-sm p-8 rounded-[2rem] border border-white/5 relative overflow-hidden group">
                ...
                <div className="text-3xl md:text-4xl font-black text-[#ccff00] mb-2 italic relative z-10">500+</div>
      ...
      <section id="services" ref={horizontalRef} className="bg-zinc-900 overflow-hidden relative">
        ...
        <div className="absolute bottom-0 left-0 w-full h-1 bg-white/5 z-20">
          <div className="scroll-progress-bar h-full bg-[#ccff00] w-full origin-left scale-x-0" />
        </div>
        ...
        <div className="pin-wrap flex h-screen items-center px-[10vw] pr-[30vw] gap-[10vw] w-max">
          ...
          <div className="flex-shrink-0 w-[80vw] md:w-[45vw]">
            <h2 className="section-heading text-7xl md:text-[10vw] font-black uppercase italic tracking-tighter leading-[0.8] mb-12">
              The <br /> <span className="text-[#ccff00]">Sinen</span> <br /> Way
            </h2>
            <div className="h-px w-40 bg-[#ccff00] mb-12" />
